...
class Province extends Private_Controller {
	...
	public function index()
	{
		...
		$js = array(
			"class",
			"modules/Administration/Province",
		);
		...
		$this->render("Administration","province_view", $data, true, true, true, $js, $css, $plugin_js, $plugin_css, "", "hold-transition skin-blue sidebar-mini", "wrapper");
	}

	public function grid($grid)
	{
		$this->load->model("Province_model");
		echo json_encode($this->Province_model->call_method($grid,"grid"));
	}

	public function ajax($action)
	{
		$this->load->model("Province_model");
		echo json_encode($this->Province_model->call_method($action,"action"));
	}
