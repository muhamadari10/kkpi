...
class Rule_access extends Private_Controller {
	...
	public function index()
	{
		...
		$js = array(
			"class",
			"modules/Administration/RuleAccess",
		);
		...
		$this->render("Administration","rule_access_view", $data, true, true, true, $js, $css, $plugin_js, $plugin_css, "", "hold-transition skin-blue sidebar-mini", "wrapper");
	}

	public function grid($grid)
	{
		$this->load->model("Rule_access_model");
		echo json_encode($this->Rule_access_model->call_method($grid,"grid"));
	}

	public function ajax($action)
	{
		$this->load->model("Rule_access_model");
		echo json_encode($this->Rule_access_model->call_method($action,"action"));
	}
