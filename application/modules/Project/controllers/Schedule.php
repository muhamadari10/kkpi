...
class Schedule extends Private_Controller {
	...
	public function index()
	{
		...
		$this->render("Project","schedule_view", $data, true, true, true, $js, $css, $plugin_js, $plugin_css, "", "hold-transition skin-blue sidebar-mini", "wrapper");
	}

	public function grid($grid)
	{
		$this->load->model("Schedule_model");
		echo json_encode($this->Schedule_model->call_method($grid,"grid"));
	}

	public function ajax($action)
	{
		$this->load->model("Schedule_model");
		echo json_encode($this->Schedule_model->call_method($action,"action"));
	}
