...
class Dashboard_model extends Eloquent{
        ...
        $this->res = array(
            "data" => array(
                "design" => array(
                    "ids"    => 1,
                    "label" => "Perencanaan",
                    "value" => ProjectMst::with(['pjp'])->select('id')->where('status_code_id', 1)
                        ->whereHas('pjp', $where_has_limiter)
                        ->where($where_has)->get()->count()
                ),
                "preparation" => array(
                    "ids"    => 2,
                    "label" => "Penyiapan",
                    "value" => ProjectMst::with(['pjp'])->select('id')->where('status_code_id', 2)
                        ->whereHas('pjp', $where_has_limiter)
                        ->where($where_has)->get()->count()
                ),
                "transaction" => array(
                    "ids"    => 3,
                    "label" => "Transaksi",
                    "value" => ProjectMst::with(['pjp'])->select('id')->where('status_code_id', 3)
                        ->whereHas('pjp', $where_has_limiter)
                        ->where($where_has)->get()->count()
                ),
                "constructionAfter2019" => array(
                    ...
                    "value" => ProjectMst::with(['pjp'])->select('id')->where('status_code_id', 4)
                        ->whereHas('pjp', $where_has_limiter)
                        ->where($where_has)->get()->count()
                ),
                "constructionOperation2019" => array(
                    ...
                    "value" => ProjectMst::with(['pjp'])->select('id')->where('status_code_id', 5)
                        ->whereHas('pjp', $where_has_limiter)
                        ->where($where_has)->get()->count()
                ),
                "constructionOperation2018" => array(
                    "ids"    => 6,
                    "label" => "Selesai",
                    "value" => ProjectMst::with(['pjp'])->select('id')->where('status_code_id', 6)
                        ->whereHas('pjp', $where_has_limiter)
                        ->where($where_has)->get()->count()
                ),
                "constructionAndOperation" => array(
                    ...
                    "value" => ProjectMst::with(['pjp'])->select('id')->where('status_code_id', 7)
                        ->whereHas('pjp', $where_has_limiter)
                        ->where($where_has)->get()->count()
                ),
            ),
    ...
    private function fund_indication_charts(){
        ...
        $sum_apbnd = ProjectMst::with(['pjp'])->select(Capsule::raw('SUM(total_fund_apbnd) AS sum_apbnd'))->whereHas('pjp',$where_has_limiter)->where($where_has)->first();
    
        $sum_bumnd = ProjectMst::with(['pjp'])->select(Capsule::raw('SUM(total_fund_bumnd) AS sum_bumnd'))->whereHas('pjp',$where_has_limiter)->where($where_has)->first();
        
        $sum_swasta = ProjectMst::with(['pjp'])->select(Capsule::raw('SUM(total_fund_swasta) AS sum_swasta'))->whereHas('pjp',$where_has_limiter)->where($where_has)->first();
    ...
    private function issue_charts(){
        ...
        // using this way if relation with JOIN
        $where_has_limiter = $this->template->limiter_access('pjp');
            ...
            "data" => array(
                "construction" => array(
                    ...
                    "value" => IssueMst::join('project_mst', 'project_mst.id', '=', 'issue.project_mst_id')
                                ...
                                ->select('issue.id')
                                ->where('issue_type_id', 1)->where($where_has_limiter)->where($where_has)->get()->count()
                ...
                "licensing" => array(
                    ...
                    "value" => IssueMst::join('project_mst', 'project_mst.id', '=', 'issue.project_mst_id')
                                ...
                                ->leftJoin('pjp', 'pjp.id', '=', 'project_mst.pjp_id')
                                ->select('issue.id')
                                ->where('issue_type_id', 2)->where($where_has_limiter)->where($where_has)->get()->count()
                ),
                "fund" => array(
                    "ids"    => 3,
                    "label" => "Pendanaan",
                    "value" => IssueMst::join('project_mst', 'project_mst.id', '=', 'issue.project_mst_id')
                                ...
                                ->leftJoin('pjp', 'pjp.id', '=', 'project_mst.pjp_id')
                                ->select('issue.id')
                                ->where('issue_type_id', 3)->where($where_has_limiter)->where($where_has)->get()->count()
                ),
                "landacquisition" => array(
                    ...
                    "value" => IssueMst::join('project_mst', 'project_mst.id', '=', 'issue.project_mst_id')
                                ...
                                ->leftJoin('pjp', 'pjp.id', '=', 'project_mst.pjp_id')
                                ->select('issue.id')
                                ->where('issue_type_id', 4)->where($where_has_limiter)->where($where_has)->get()->count()
                ),
                "planning" => array(
                    "ids"    => 5,
                    ...
                    "value" => IssueMst::join('project_mst', 'project_mst.id', '=', 'issue.project_mst_id')
                                ...
                                ->leftJoin('pjp', 'pjp.id', '=', 'project_mst.pjp_id')
                                ->select('issue.id')
                                ->where('issue_type_id', 5)->where($where_has_limiter)->where($where_has)->get()->count()
                ),
            ),
            "status" => true,
            "message" => "Success"
    ...
    private function project_detail_list(){
        ...
        // using this way if relation with JOIN 'pjp'
        $where_has_limiter = $this->template->limiter_access('pjp');
    ...
    private function resume_project(){
        ...
        // use this way if relation not using JOIN 'not declare pjp' 
        $where_has_limiter = $this->template->limiter_access();
