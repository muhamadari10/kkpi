...
class Template
{
    ...
    public function limiter_access($add_table='')
    {

        
        $table = ($add_table != '')?$add_table.'.':'';
        ...
        if ( ($this->get_session('group_name') == 'manager') || $this->get_session('group_name') == 'coordinator' ) {
            ...
            $where = function($q) use($pjp_id, $table) {
                
                $q->where($table . 'id', $pjp_id);
    ...
    private function init_menu()
    {
        ...
        foreach($head_menu as $key => $val) {
            ...
            if($this->CI->session->userdata('pjp_name') !== 'ALL' && $user_permission->menu->name === 'Administrasi'){
                $html .= '';
            }else{
                if($count_num > 0) {
