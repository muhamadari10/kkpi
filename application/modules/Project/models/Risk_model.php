...
class Risk_model extends Eloquent{
    ...
    private function risk_list(){
        ...
        $q_data = RiskMst::with(['project_mst', 'issue_type', 'project_mst.province', 'severity_level', 'frequency_level', 'activity', 'phase'])
                    ->where($where_has);
                        ...
                        && $v['data'] != 'stakeholder_name'
                            ...
                            if ( $v['data'] == 'project_name' ) {
                                ...
                                $ds->whereHas('project_mst', function($q) use($v, $search){
                                    $q->orWhere('project_name', 'LIKE', '%'.$search.'%');
                                });
                            ...
                            } else if ( $v['data'] == 'severity_level_risk' ) {
                                
                                $ds->whereHas('severity_level', function($q) use($v, $search){
                                    $q->orWhere('severity_level_name', 'LIKE', '%'.$search.'%');
                                });

                            } else if ( $v['data'] == 'frequency_level_risk' ) {
        ...
        foreach ($data_in as $key => $value) {
            ...
            $this->data[$key]['risk_description'] = $value->risk_description;
            ...
            $this->data[$key]['severity_level_risk'] = nl2br('Level ' . $value->severity_level->level . '<br/>' . $value->severity_level->severity_level_name);
            $this->data[$key]['frequency_level_risk'] = $value->frequency_level->frequency_level_name;
            ...
            $this->data[$key]['case_risk_date'] = date('m/d/Y', strtotime($value->case_risk_date));
            $this->data[$key]['planning_solve_risk_date'] = date('m/d/Y', strtotime($value->planning_solve_risk_date));
        ...
        $val   = $_GET['search']['value'];
        if(!empty($val))
            $count = RiskMst::with(['project_mst', 'issue_type', 'project_mst.province', 'severity_level', 'frequency_level', 'activity', 'phase'])
            ->where($where_has)
        ...
        $this->res = array(
            'recordsTotal' => isset($count)?$count->count():intval(
                RiskMst::with(['project_mst', 'issue_type', 'project_mst.province', 'severity_level', 'frequency_level', 'activity', 'phase'])
    ...
    private function create_risk()
    {
        ...
        try {
            ...
            // $check_risk_exist = RiskMst::select('id')
            //                         ->where('risk_description', $_POST['riskDescription'])
            //                         ->get();

            // if ( $check_risk_exist->count() < 1 ) {

                $risk_store = new RiskMst();
                $risk_store->project_mst_id = $_POST['projectOpt'];
                $risk_store->risk_description = $_POST['riskDescription'];
                $risk_store->impact_duration = $_POST['impactDuration'];
                $risk_store->severity_level_id = $_POST['severityLevelOpt'];
                $risk_store->frequency_level_id = $_POST['frequencyLevelOpt'];
                $risk_store->activity_id = $_POST['activityOpt'];
                $risk_store->phase_id = $_POST['phaseOpt'];
                $risk_store->issue_type_id = $_POST['issueTypeOpt'];
                $risk_store->stakeholder = json_encode($_POST['stakeholderOpt']);
                $risk_store->planning_solve_risk_date = date('Y-m-d', strtotime($_POST['planningSolveRiskDate']));
                $risk_store->case_risk_date = date('Y-m-d', strtotime($_POST['caseRiskDate']));
    ...
    private function update_risk()
    {
        ...
        try {
            ...
            // if ( $check_risk_exist->count() < 1 ) {
                ...
                $risk_store = RiskMst::find($_POST['riskId']);
                $risk_store->project_mst_id = $_POST['projectOpt'];
                $risk_store->risk_description = $_POST['riskDescription'];
                $risk_store->impact_duration = $_POST['impactDuration'];
                $risk_store->severity_level_id = $_POST['severityLevelOpt'];
    ...
    private function delete_risk()
    {

        $risk_del = RiskMst::find($_POST['id']);
        $risk_del->delete();
